import { Fragment, useState, FC, useEffect, useRef } from "react";
...
const LoginForm: FC<Props> = ({ extraClass, children }) => {
  ...
  // Cleanup any pending timers to avoid setState on unmounted warnings
  useEffect(() => {
    return () => {
  ...
  return (
    <>
      ...
      <div className="mb-12">
        ...
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray500 mb-2">Select Branch</label>
          <div className="flex gap-2 items-center">
            <select
              className="px-3 py-2 border border-gray300 rounded-md w-full"
              value={branchId}
            ...
            <button
              type="button"
              className="px-4 py-2 border rounded bg-gray500 text-white hover:bg-gray600"
              onClick={() => {
                if (!branchId) return;
                setCookie("branch_id", branchId, { path: "/" });
                // Notify app immediately for cart price refresh
                if (typeof window !== "undefined") {
                  window.dispatchEvent(
                    new CustomEvent("branch-changed", { detail: { branch_id: branchId } })
                  );
                }
                // Reload to apply product filtering by branch across pages
                router.reload();
              }}
            >
              Apply
          ...
          <p className="text-xs text-gray400 mt-1">Products will reflect stock/prices for the selected branch.</p>
        </div>
      </div>
